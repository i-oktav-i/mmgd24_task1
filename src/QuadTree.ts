...
export class QuadTree {
  constructor(
    ...
    private capacity = 9,
    private bounds: [topLeft: Vector, bottomRight: Vector]
  ) {
    const topLeft: Collider[] = [];
    const topRight: Collider[] = [];
    const bottomLeft: Collider[] = [];
    ...
    const center = new Vector(
      bottomRightBound.x - topLeftBound.x,
      topLeftBound.y - bottomRightBound.y
    );

    colliders.forEach((collider) => {
      const horizontal = collider.relativeToHorizontal(center.y);
      const vertical = collider.relativeToVertical(center.y);
    ...
    if (topLeft.length > capacity) {
    }

    [topLeft, topRight, bottomLeft, bottomRight].forEach((group) => {
      if (group.length > capacity) {
      }
    });
  }
